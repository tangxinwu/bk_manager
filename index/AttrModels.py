...
class JSONAttr:
    ...
    def __get__(self, instance, owner):
        return self.result[self.storage_name]
...
class TitleValid(Validted):
    ...
    def validte(self, instance, value):
        if value:
            ...
            modified_value.update({"x": "center"})  # 标题居中
...
class LegendValid(Validted):
    ...
    def validte(self, instance, value):
        if isinstance(value, list):
            ...
            modified_value.update({'type': 'scroll', 'right': 50, 'top': 30})  # 默认垂直显示图列方式
            return modified_value
        else:
